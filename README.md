...
who typed a fingerprint would have believed the connection was checked.

Every test here missed it, because they all built a client and inspected it.
The downstream test that found it pinned a **wrong** certificate and expected the
walk to fail — and it passed.

**0.1.3 was two contracts and a guard.**
...
| Tests | **721** collected with PyYAML installed, **695** with nothing. The difference is exactly `tests/test_action.py`, which reads the shipped `action.yml` and skips as a whole module when PyYAML is absent — so CI installs it; the `[detect]` extra adds an engine canary on top of both |
